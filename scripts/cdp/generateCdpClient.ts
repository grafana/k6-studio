...
import { camelCase, toEventTypeName, toResultTypeName } from './utils'
...
function compareOptional(a: cdp.Property, b: cdp.Property) {
    ...
    return ClassBuilder.define()
      ...
      .reduce(domain.commands ?? [], (b, command, self) => {
        return b.method(camelCase(command.name), (b) => {
          const params = command.parameters?.toSorted(compareOptional) ?? []
          ...
          return b
            .reduce(params, (b, param) => {
              const name = escape(param.name)
              const type = toTypeWithRef(param, domain.domain)

              return param.optional
                ? b.optional(name, type)
                : b.param(name, type)
            })
            .returns(t.promise(returnType))
            .body((params) => {
              const paramsExpr = new ObjectBuilder()
                .reduce(Object.entries(params), (b, [name, expr]) => {
                  return b.property(name, expr)
                })
                .done()
