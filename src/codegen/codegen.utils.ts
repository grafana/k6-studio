...
/**
 * The "Generated by Grafana k6 Studio" prefix is load-bearing: cloud usage
 * reporting (dbt's rpt_k6_daily_usage model) prefix-matches it to flag
 * Studio-made test runs, and matches the wizard suffix the same way for
 * wizard-made ones. Change the wording only in lockstep with that model.
 */
export function generateScriptHeader(wizardUsed: boolean) {
  const header = `Generated by Grafana k6 Studio (${__APP_VERSION__}) on ${new Date().toISOString()}`
